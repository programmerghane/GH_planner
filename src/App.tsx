...
export default function App() {
    ...
    async function init() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setState(data);
          // Set cached theme if applicable
          if (data.profile?.themeId) {
            const match = PREMIUM_THEMES.find((t) => t.id === data.profile.themeId);
            if (match) setActiveTheme(match);
          }
          if (data.profile?.language) {
            setLangState(data.profile.language);
          }
        }
      } catch (err) {
  ...
  const handleUpdateTasks = (updatedTasks: Task[], xpReward = 0) => {
    ...
    if (xpReward > 0) {
      addXP(xpReward, "task", `Planner schedule update`);
    } else {
    ...
    return (
      <AuthSystem
        ...
        onLoginSuccess={(profile, fullState) => {
          localStorage.setItem("gh_planner_user_id", profile.userId);
